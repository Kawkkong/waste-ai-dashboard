...
def analyze_frame(img, camera):
    ...
    # ==================================================
    #
    # ใช้ค่าเดิมของโครงงาน
    #
    # imgsz = 1280
    # conf = 0.20
    # retina_masks = True
    #
    # ไม่เปลี่ยน confidence
    # ==================================================
    ...
    if result.masks is not None:
        ...
        for mask in masks:
            ...
            # ------------------------------------------------
            # retina_masks=True ควรได้ mask ขนาดภาพต้นฉบับ
            #
            # ถ้าขนาดไม่ตรง จะไม่ resize เงียบ ๆ เพราะการ resize
            # อาจทำให้พื้นที่ที่ใช้คำนวณ WAR เปลี่ยนโดยไม่รู้ตัว
            # ------------------------------------------------

            if mask.shape != (h, w):

                raise ValueError(
                    "ขนาด Segmentation Mask ไม่ตรงกับภาพต้นฉบับ: "
                    f"mask={mask.shape}, image={(h, w)}"
                )


            # ------------------------------------------------
            # Convert probability mask -> binary mask
            # ------------------------------------------------
    ...
    # พื้นที่ขยะทั้งหมดก่อนตัดด้วย ROI
    ...
    return {
        ...
        # ----------------------------------------------
        # Diagnostic values
        # ใช้ตรวจสอบว่า WAR ต่ำเพราะ mask น้อยจริงหรือไม่
        # ----------------------------------------------
